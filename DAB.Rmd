...
The same line of reasoning also works for the PCA model. The performance is still smaller than the models obtained trough variable selection. Our best model is hence the one found with aic.
...
The function performance metrics computes in addittion key performance metrics such as accuracy, precision, recall (sensitivity), and F1 score to evaluate the model's performance at each threshold.
```{r, warning=FALSE}
# Thresholds choices
...
gain by achieving a good balance between sensitivity and precision and is hence the optimal solution.