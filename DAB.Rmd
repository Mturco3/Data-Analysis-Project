...
A point of concern is the presence of 'Unknown' values within 'Educational_Level' and 'Marital_Status'. These entries, which represent a lack of data, might complicate our analysis. They introduce a level of uncertainty that could distort the models' predictions, making them less reliable. It’s crucial to address these anomalies, perhaps by exploring data imputation techniques or sensitivity analyses, to mitigate their potential impact on the predictive performance of our models.
...
Using set.seed(123) guarantees that anyone rerunning the analysis will get the same results, making our work reproducible and reliable. It's essential for any steps that involve random choices, like dividing data into training and test sets.
...
# On the train_set, we see a better performance of roc_aic2 and roc_aic3 models
#in respect to the others. AUC = 0.9169

```
On the train_set, the two models have the same ROC curve with the same AUC=0.918.
...
```
On the train_set, the models has AUC=0.866.

### AIC and BIC model (VALIDATION SET)
...
# AUC scores
(auc_glm_aic1 <- roc_out_aic1$auc)

(auc_glm_bic1 <- roc_out_bic1$auc)

#From here we see that also on the validation set, the best performing models are
#roc_aic2 and roc_aic3. AUC = 0.9201
...
```{r}
TestData = read.csv2("C:/Users/Thebrickster21/Desktop/DataAnalProject/bank_accounts_test.csv", 
                 header = T, 
                 sep = ",", 
                 colClasses = "character")
...
predicted_prob <- predict(logit_fit_aic2,
                        newdata = TestData,
                        type = "response")
predicted_prob <- as.factor(ifelse(predicted_prob > tt, "yes", "no"))
